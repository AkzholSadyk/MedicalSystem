...
class FaceIDService:
    ...
    def verify_face_against_user(
        self, uploaded_image_data: bytes, user: User
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            confidence = (
                max(0, min(1, 1 - (distance / threshold))) if threshold > 0 else 0
            )
    ...
    def verify_face_against_all_users(
        self, uploaded_image_data: bytes, db: Session
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            users = (
                db.query(User)
                .filter(User.avatar.isnot(None), User.deleted_at.is_(None))
                .all()
            )
