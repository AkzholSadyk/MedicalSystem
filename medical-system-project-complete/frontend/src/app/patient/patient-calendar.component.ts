...
@Component({
  ...
  template: `
  <h2>{{ 'CALENDAR.PATIENT_TITLE' | translate }}</h2>
    ...
    this.apptService.getCalendarAppointments(fromS, toS).subscribe({
      next: (data:any[]) => {
        if (data && data.length > 0) {
          ...
          // fallback: fetch patient's own appointments and map them
          this.apptService.getPatientAppointments().subscribe({
            next: (appts) => {
              this.events = appts.map(a => ({
                ...
                doctor_name: a.doctor_name || (a.doctor ? (a.doctor.first_name + ' ' + a.doctor.last_name) : undefined),
                patient: a.patient,
                doctor: a.doctor,
                duration: a.duration
              }));
      ...
      error: (err) => {
        ...
        // on error, fallback to patient appointments
        this.apptService.getPatientAppointments().subscribe({
          next: (appts) => {
            this.events = appts.map(a => ({
              id: a.id,
